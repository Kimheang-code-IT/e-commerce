...
const {
    ...
    totalRows,
    selectedUser, users, confirmConfig,
    userFormFields, columns, roleItems, selectedRoles,
    getDropdownActions, handleSaveRequest, finalizeAction, handleAddNew, canCreate, isLoading, isMutating,
    ...
    <LayoutAppHeader :title="$t('pages.userManagement.title')" show-datepicker>
        ...
        <div class="flex-1 min-h-0 overflow-hidden">
            <TableApptable :title="$t('pages.userManagement.tableTitle')" v-model:row-selection="rowSelection"
                ...
                :data="users" :columns="columns" :selectable="true" :total-rows="totalRows" :loading="isLoading" :get-row-actions="getDropdownActions">
        ...
        <CommonAppSlideoverForm
            ...
            @submit="onSubmitUser"
        />
        <CommonAppModalCURD v-model:open="isConfirmOpen" v-bind="confirmConfig" :loading="isMutating" @submit="finalizeAction" />
    </LayoutAppHeader>
</template>
